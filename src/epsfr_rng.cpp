...
/*
Following is inspired from Fisher-Yates fast shuffling algorithm.
*/
vector<int>* t_rng::fast_permute_indices(int i_min, int i_max)
{
	vector<int>* permuted_indices = new vector<int>();

	// Fill the remaining indices.
	for(int cur_i = i_min; cur_i < i_max; cur_i++)
	{
		// do not add the index to exclude to the bag.
		permuted_indices->push_back(cur_i);
	}

	int n_elements = permuted_indices->size();
	for(int i = n_elements-1; i > 0; i--)
	{
		// Select an index 0 <= rand_num <= i.
		double rand_num = floor((i + 1) * this->random_double_ran3());

		int rand_i_bag = (int)rand_num;

		// Swap the last element with the randomly chosen one.
		int temp = permuted_indices->at(i);
		permuted_indices->at(i) = permuted_indices->at(rand_i_bag);
		permuted_indices->at(rand_i_bag) = temp;
	} // i loop.
...
vector<int>* t_rng::permute_indices(int i_max, int i_to_exclude_from_first_position)
{
	...
	while(!index_bag->empty())
	{
		...
		int rand_i_bag = (int)rand_num % (int)index_bag->size();
